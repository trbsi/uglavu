...
* Copyright (c) 2016 Modified by Rico Macchi; Licensed MIT
 */
(function() {
  (function($) {
	...
	var elementzero = this.element[0].tagName.toLowerCase();
        } else {
          return null;
        }
      };
	...
	var selfgetpropertagname = [self.getProperTagName()];
            }
          ]
        ];
        for (i = 0, len = ref.length; i < len; i++) {
      ...
      Selectorator.prototype.generateRecursive = function() {
        ...
        index = parent.children(this).index(this.element);
        selector = "child(" + index + ")";
        if (parentSelector !== '') {

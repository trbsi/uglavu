...
export const mount = el => {
  if (el.querySelector('.notice-blocksy-plugin-root')) {
    render(
      <Notification
        initialStatus={
          el.querySelector('.notice-blocksy-plugin-root').dataset.pluginStatus
        }
        url={el.querySelector('.notice-blocksy-plugin-root').dataset.url}
        pluginUrl={
          el.querySelector('.notice-blocksy-plugin-root').dataset.pluginUrl
        }
        pluginLink={
  ...
  ;[...document.querySelectorAll('[data-dismiss]')].map(el => {
    el.addEventListener('click', e => {
      ...
      el.closest('.notice-header-builder').remove()

      $.ajax(ajaxurl, {
        type: 'POST',
        data: {
          action: 'blocksy_dismissed_notice_header_builder'
        }
      })
    })
  })

      ...
      extractValue: ({
        ...
        if (color.indexOf('rgb') > -1) {
          const rgb_array = str_replace(
            ...
            str_replace(
              ')',
              '',
              str_replace('rgba(', '', str_replace(' ', '', color))
            )
          ).split(',')

          color = `#${parseInt(rgb_array[0], 10).toString(16)}${parseInt(
            rgb_array[1],
            10
          ).toString(16)}${parseInt(rgb_array[2], 10).toString(16)}`
        ...
        return `url("${str_replace(
          ...
          str_replace(
            'COLOR',
            color,
            ct_localizations.customizer_sync.svg_patterns[background_pattern] ||
              ct_localizations.customizer_sync.svg_patterns['type-1']
          )
        )}")`
      }
    },
